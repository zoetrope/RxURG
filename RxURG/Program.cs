...
namespace RxURG
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //Disposeが呼ばれるタイミングの確認();

            ParseMeasuredData(_data.Split('\n').ToList());
        ...
        {
            var sensor = new Subject<string>();
            var observable = GetSensor(sensor);
        ...
        private static IObservable<string> GetSensor(ISubject<string> sensor)
        {
        ...
        private static void コマンド送受信()
        {
            ...
            var commandObserver =
                Observable.Defer(() =>
                    sensor.TakeWhile(s => s != string.Empty)
                        .Aggregate(new List<string>(), (l, s) => { l.Add(s); return l; }))
                .Repeat();
            
            var last = commandObserver
                .Where(l => l.First().StartsWith("VV"))
                .Take(1)
                .PublishLast();
            last.Connect();
            ...
            var version = last.Timeout(TimeSpan.FromSeconds(3)).Wait();


            Console.WriteLine("VV Received.");
            
            commandObserver.Where(l => l.First().StartsWith("MD"))
                .Subscribe(x => Console.WriteLine("MD Received."));

            commandObserver.Where(l => l.First().StartsWith("PP"))
                .Subscribe(x => Console.WriteLine("PP Received."));
            
            commandObserver.Subscribe();
        ...
        public static object ParseMeasuredData(List<string> messageList)
        {
            var distance = Decode(messageList);
            var points = distance.Select(PolarToCartesian);

            return points.ToList();
        }

        public static IEnumerable<int> Decode(List<string> messageList)
        {
            // 最初の3行はエコーバック、エラーコード、タイムスタンプなので飛ばす。
            // 4行目以降は、チェックサムを取り除いて結合する。
            ...
            var distance = Encoding.ASCII.GetBytes(data)
                ...
                .Select(x => (x[0] - 0x30) * 4096 + (x[1] - 0x30) * 64 + (x[2] - 0x30)); // 距離データに変換
        ...
        const double Resolution = 0.352;

        public static Point PolarToCartesian(int r, int index)
        {
        ...
        private static void Disposeが呼ばれるタイミングの確認()
        {
            //var sensor = new DummySensor();

            var sensor = new Subject<byte>();
            ...
            var lineSubject = new Subject<string>();

            lineSubject.Subscribe();
            


            Console.WriteLine("len = {0}",_data.Length);
            ...
            foreach (var i in Enumerable.Range(0,1000))
            {
                foreach (var x in Encoding.ASCII.GetBytes(_data))
                {
                    sensor.OnNext(x);
                    ...
                    if (x == '\0')
                    {                        
                        lineSubject.OnNext(Encoding.ASCII.GetString(list.ToArray()));
                        list.Clear();
                    }
                    */
                    //sensor.Notify(x);
        ...
        static void foo()
        {
            ...
            Observable.Defer(
                () => subject.TakeWhile(x => x != 0)
                    .Aggregate(new List<int>(), (l, i) => { l.Add(i); return l; }))
                .Repeat()
                .Select(l => l.Sum())
                .Subscribe(Console.WriteLine, _ => Console.WriteLine("OnCompleted"));
...
//        {
//
//            var serialPort = new ObservableSerilaPort("");
//
//
//            serialPort.TakeWhile(b => b != '\n')
//                .Aggregate(new List<byte>(), (l, b) =>
//                {
//                    l.Add(b);
//                    return l;
//                })
//                .Select(l => Encoding.ASCII.GetString(l.ToArray()));
//
//
//        }
    ...
    class SerialSubject : IObservable<string>
    {
        ...
        private int _restLength = 0;
        private byte[] _restBuffer = new byte[4096 * 2];

        public void Receive(IObserver<string> observer)
        {
            Observable.Start(() =>
            {
                while (true)
                {
                    ...
                    int len = serialPort.Read(buffer, 0, 4096);

                    if (len == -1) return;

                    int prev = 0;

                    // 前回の残りのデータと今回の受信データを結合する
                    Array.Copy(buffer, 0, _restBuffer, _restLength, len);

                    for (int i = 0; i < len + _restLength; i++)
                    {
                        if (_restBuffer[i] == '\n')
                        {
                            // 改行文字が現れたら、これまでのデータを文字列として返す
                            byte[] ret = new byte[i - prev];
                            Array.Copy(_restBuffer, prev, ret, 0, i - prev);
                            var message = Encoding.ASCII.GetString(ret);
                            observer.OnNext(message);
                            prev = i + 1;
                        }
                    }

                    if (prev != len + _restLength)
                    {
                        // 残りのデータを保存しておく
                        _restLength = len + _restLength - prev;
                        byte[] temp = new byte[_restLength];
                        Array.Copy(_restBuffer, prev, temp, 0, _restLength);
                        Array.Copy(temp, 0, _restBuffer, 0, _restLength);
                    }
                    else
                    {
                        _restLength = 0;
    ...
    internal class DummySerialPort
    {
        ...
        public int Read(byte[] buffer, int offset, int size)
        {
            ...
            {
                case 1:
                    var data1 = Encoding.ASCII.GetBytes("abcdefh\nggggg");
                    data1.CopyTo(buffer, 0);
                    return data1.Length;
                case 2:
                    var data2 = Encoding.ASCII.GetBytes("abcdefh\nggggg");
                    data2.CopyTo(buffer, 0);
                    return data2.Length;
                default:
                    break;
            }
